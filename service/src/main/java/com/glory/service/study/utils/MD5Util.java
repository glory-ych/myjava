...
public class MD5Util {
    ...
    public static String getFileNameMD5(int userId) {
        long random = (long) (Math.random() * (9999 - 1000 + 1)) + 1000;  //四位随机数
        long currentTime = System.currentTimeMillis();  //时间戳
        String filename = "" + currentTime + userId + random;
        String filenameMD5 = MD5Util.getMD5(filename);  //MD5转码 时间戳+用户ID+四位随机数
        return filenameMD5;
    }
